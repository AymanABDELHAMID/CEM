"""
Ayman MAHMOUD - CEM algorithm module
"""
...
def classification(g, pi, X):
    """
    Each datapoint in X will have one partition z that is based on the maximum value of the posterior probability
    :param g: g is a numpy.ndarray of shape (k, n) containing the posterior probabilities for each data point in each cluster
    :return: Z_i which is the classification of each datapoint to the most likely cluster based on (t_ik)
    """
    # create an array w/ kxn dimensionality
    n, d = X.shape
    k = pi.shape[0]
